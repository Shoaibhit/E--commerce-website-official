...
function get_user_order_details(){
  ...
  $get_details="select * from `user _table` where username='$username'";
    ...
    if(!isset($_GET['edit_account'])){
      if(!isset($_GET['my_orders'])){
       if(!isset($_GET['Delete_account'])){
        ...
        if($row_count>0){
          echo"<h3 class='text-center text-success mt-5 mb-2 text-success my-5'>You have <span class='text-danger'>$row_count</span> pending orders</h3>
          <p class='text-center'><a href='profile.php?my_orders' class='text-dark'>Orders Details</a></p>";
        }else{
          echo"<h3 class='text-center text-success mt-5 mb-2 text-success my-5'>You have zero pending orders</h3>
